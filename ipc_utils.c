...
Response * create_shared_memory(char * name, int size) {
    int shm_fd = 0;
    ...
    if(ftruncate(shm_fd, size) == ERROR) {
    ...
    if ( (pointer_to_shm = (Response *) mmap(NULL, size,PROT_WRITE, MAP_SHARED, shm_fd, 0)) == MAP_FAILED ){
